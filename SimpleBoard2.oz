...
fun {UpdateDistsPartForPutVacant
   ...
   fun {Recurs Lst}
      if Lst==nil then
          ...
          R#C#Dist|Rest = Lst
          in
             if VR#VC\=R#C andthen Dist<@NewDist then
                NewR:=R  NewC:=C  NewDist:=Dist  
             end
...
class Board 
   ...
   in % put
     if R#C \= @LastPut then 
        ...
        for I in 1..self.influenceRadius do  % reevaluate dists for everything
           {self processManhattanDistance(           %  within influenceRadius  
              R 
              C
              I
   ...
   in % getManhattan
      for I in 1..Radius do 
         {self processManhattanDistance(R C I AddToList)}
      end
      WithinRadius = @List
   end % getManhattan

   meth processManhattanDistance(R C Dist Proc)
